...
**重要：** 
- 把 `DB_PASSWORD` 改成你在第三部分保存的数据库密码
- 把 `DB_USERNAME` 和 `DB_DATABASE` 改成你实际创建的数据库用户名和数据库名
- 建议修改 `JWT_SECRET` 为随机字符串（可使用在线生成工具）
...
**注意：** 
- 默认配置已经适配宝塔部署，通常不需要修改
- `/api` 会通过 Nginx 反向代理到后端服务（端口 3000）
- 如果使用域名，确保在第七部分正确配置 Nginx
